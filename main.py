...
def ex4():
    all_dates_all_fields = np.full([14, 76, 4016], "will change")
    path = 'C:/Users/dan-client/Downloads/Bar_python_exercise/ex1'
    folder_index = 0
    for root, dirs, files in os.walk(path):
        for dir in dirs:
            ...
            for root1, dirs1, files1 in os.walk(path + '/' + dir):
                for file in files1:
                    file_in_list = np.load(path + '/' + dir + '/' + file, allow_pickle=True)["arr_0"]
                    padding = np.append(file_in_list, [np.nan] * (4016 - file_in_list.shape[0]))
                    for num_index in range(4016):
                        all_dates_all_fields[folder_index, file_index, num_index] = padding[num_index]
                    file_index += 1
            folder_index += 1
